...
enum Qwen3VLVision {
    ...
    final class MLP: Module, UnaryLayer {
        ...
        init(dim: Int, hiddenDim: Int) {
            ...
            // mlx-vlm qwen3_vl uses GELU(approx="tanh"); .fast is the sigmoid approximation
            // (x*sigmoid(1.702x)) and diverges every block — must be the tanh approximation.
    ...
    final class VisionModel: Module {
        ...
        /// Bisect helper: pre-block tensors (patch_embed, pos_embed, rotary).
        func debugPreBlocks(_ pixelValues: MLXArray, gridTHW: [THW])
            -> (MLXArray, MLXArray, MLXArray)
        {
            (patchEmbed(pixelValues), positionalEmbeddings(gridTHW),
...
enum Qwen3VLLanguage {
    ...
    static func applyMultimodalRotary(
        ...
        let qEmbedded = (q * cos) + (qwenRotateHalf(q) * sin)
        let kEmbedded = (k * cos) + (qwenRotateHalf(k) * sin)
...
public final class Qwen3VL: Module {
    ...
    public func lastHiddenState(
        ...
        if let pixelValues, let framesList = imageGridTHW, !framesList.isEmpty {
            ...
            if !deepstackOutputs.isEmpty {
                deepstackEmbeds = deepstackOutputs.map { layerFeatures in
                    let si = cumulativeSplitIndices(from: splits)
                    return concatenated(layerFeatures.split(indices: si)).asType(textEmbeds.dtype)
                }
            }
        }
    ...
    public struct EditDebug {
        public let visHidden: MLXArray
        public let deepstack: [MLXArray]
        public let merged: MLXArray
        public let positionIds: MLXArray
    }

    public func debugEdit(inputIds: MLXArray, pixelValues: MLXArray, imageGridTHW: [THW]) throws
        -> EditDebug
    {
        let textEmbeds = languageModel.model.embedTokens(inputIds)
        let (visionHidden, deepstackOutputs) = visionModel(pixelValues, gridTHW: imageGridTHW)
        let mergeSize = config.visionConfiguration.spatialMergeSize
        let splits = imageGridTHW.map { $0.product / (mergeSize * mergeSize) }
        let si = cumulativeSplitIndices(from: splits)
        let flat = concatenated(visionHidden.split(indices: si)).asType(textEmbeds.dtype)
        let (merged, _) = try mergeInputIdsWithImageFeatures(
            imageFeatures: flat, inputEmbeds: textEmbeds, inputIds: inputIds,
            imageTokenIndex: config.imageTokenIndex, videoTokenIndex: config.videoTokenIndex)
        let deep = deepstackOutputs.map {
            concatenated($0.split(indices: si)).asType(textEmbeds.dtype)
        }
        let (pos, _) = Qwen3VLLanguage.getRopeIndex(
            inputIds: inputIds, imageGridTHW: imageGridTHW, videoGridTHW: nil,
            spatialMergeSize: mergeSize, imageTokenId: config.imageTokenIndex,
            videoTokenId: config.videoTokenIndex, visionStartTokenId: config.visionStartTokenId,
            attentionMask: nil)
        return EditDebug(visHidden: visionHidden, deepstack: deep, merged: merged, positionIds: pos)
    }

    public func debugVisionPre(pixelValues: MLXArray, imageGridTHW: [THW])
...
/// QwenVL.rotateHalf (inlined; the MLXVLM original is module-internal).
func qwenRotateHalf(_ x: MLXArray) -> MLXArray {
    let index = x.dim(-1) / 2
